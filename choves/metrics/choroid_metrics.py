...
from .choroid_utils import (extract_bounds, curve_length, curve_location, generate_imgmask, get_trace,
...
def compute_choroid_measurement(reg_mask, 
                               ...
                               verbose=0):
    """
    Compute all choroid measurements of interest, that is choroidal thickness and area using the reg_mask and 
    CVI (and optional vessel/stromal area) using the vess_mask.
    ...
    scale: Microns-per-pixel in x and y directions. Default setting is Heidelberg scaling 
    ...
    if vess_mask is not None:
        ...
        for x in xchor:
            ...
            bmap_col = vess_mask[col[:,1], x]
            
            vessel_area += bmap_col.sum()
            vessel_pixels.append(col[bmap_col == 1])
            stroma_area += np.count_nonzero(np.abs(bmap_col-1).sum())
        ...
        choroid_evi = np.round(stroma_area/total_chor_area, 5) # 1 - choroid_cvi
        choroid_vessel_area = np.round(1e-6 * micron_area * vessel_area, 6)
        choroid_interstitial_area = np.round(1e-6 * micron_area * stroma_area, 6) # choroid_area - choroid_vessel_area
...
def compute_area_enclosed(traces, 
    ...
    ---------------------
        ...
        scale (2-tuple) : Tuple storing pixel_x-pixel_y-micron scalar constants.
